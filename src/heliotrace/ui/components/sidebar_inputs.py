...
from heliotrace.config import DEFAULT_OBS_ROWS
from heliotrace.models.schemas import GCSParams, SimulationConfig
...
def render_sidebar() -> tuple[SimulationConfig, GCSParams, pd.DataFrame, bool, bool]:
    ...
    with st.sidebar:
        ...
        # ------------------------------------------------------------------ #
        target = render_target_selector(obs_time=cme_launch_day)
