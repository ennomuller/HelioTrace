...
def build_propagation_comparison_figure(
    ...
    if dbm_t is not None:
        ...
        fig.add_vline(x=dbm_t, line=dict(color=_COLORS["DBM"], width=1.5, dash="dash"), row=1, col=1)
        fig.add_vline(
            x=dbm_t,
    ...
    if modbm_t is not None:
        ...
        fig.add_vline(x=modbm_t, line=dict(color=_COLORS["MODBM"], width=1.5, dash="dash"), row=1, col=1)
        fig.add_vline(
            x=modbm_t,
